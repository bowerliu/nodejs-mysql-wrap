class M {
	...
	getById (id,cb) {
		this.conn.selectQuery(this.table, {id:id}, cb)
	};
	getData(options,callback){
		...
		this.conn.selectQuery(this.table, where, cb, limit,orderby, groupby,columns,join_sign,eq_sign, options.join, options._on,options.no_cache,options.cache_key)
	};
	getAllByWhere (where,cb,limit,orderby) {
		...
		this.conn.selectQuery(this.table, where, cb, limit,orderby)
	};
	...
	exec(cb){
		...
		this.conn.selectQuery(this.table, this._where, cb, this._limit,this._orderby, this._groupby,this.cols,this.join_sign,this.eq_sign, this.join, this._on,this.no_cache,this.cache_key)
	}
}
